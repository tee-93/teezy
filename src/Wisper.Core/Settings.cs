...
public sealed record WisperSettings
{
    ...
    public static WisperSettings Load(string? path = null)
    {
        path ??= DefaultPath;
        try
        {
            return File.Exists(path)
                ? JsonSerializer.Deserialize<WisperSettings>(File.ReadAllText(path), Json) ?? new()
                : new WisperSettings();
        }
