...
public class DictationControllerTests
{
    ...
    private static Harness Build(WisperSettings? settings = null, string? dictionaryFile = null)
    {
        ...
        var path = dictionaryFile ?? Path.Combine(Path.GetTempPath(), $"wisper-{Guid.NewGuid():N}.txt");
        var store = new DictionaryStore(path);

        var effective = settings ?? new WisperSettings { MinimumHoldMilliseconds = 0 };
    ...
    {
        var h = Build(new WisperSettings { MinimumHoldMilliseconds = 10_000 });
    ...
    public async Task DictionaryAppliesEvenWithCleanupOff()
    {
        ...
        var path = Path.Combine(Path.GetTempPath(), $"wisper-{Guid.NewGuid():N}.txt");
        await File.WriteAllLinesAsync(path, ["cloud code -> Claude Code"]);

        var h = Build(new WisperSettings { CleanupEnabled = false, MinimumHoldMilliseconds = 0 }, path);
